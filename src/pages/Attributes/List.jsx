...
import CreateAttributeModal, { ATTRIBUTE_TYPES } from './CreateModal';
...
const AttributesList = () => {
    ...
    const fetchTemplatesAndGroups = async () => {
        try {
            ...
            if (!selectedTemplateId && !searchParams.get('templateId') && normalizedTemplates.length > 0) {
    ...
    const handleCreate = async ({ name, type }) => {
        ...
        try {
            ...
            const created = await createProductAttribute(payload);
            showAlert('success', 'Attribute created successfully');
            setShowCreate(false);
            if (created?.id) {
                navigate(`/attributes/${created.id}?templateId=${selectedTemplateId}`);
            } else {
                fetchAttributes(selectedTemplateId);
            }
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            if (selectedTemplateId) fetchAttributes(selectedTemplateId);
        } catch (error) {
    ...
    const columns = [
        ...
        },
        {
            ...
            header: 'Required',
            render: (value) => (
                <Badge variant={value ? 'warning' : 'default'}>{value ? 'Yes' : 'No'}</Badge>
            ),
        },
        {
            key: 'group',
            header: 'Group',
            render: (value, row) => (
                <span className="text-slate-900 dark:text-white">{getGroupName(row.groupId)}</span>
            ),
        },
        {
            key: 'actions',
            ...
            render: (value, row) => (
                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    <button
                        onClick={() => navigate(`/attributes/${row.id}?templateId=${selectedTemplateId}`)}
    ...
    return (
        <CatalogPageFrame>
            <CatalogHero
                ...
                actions={
                    <Button onClick={() => setShowCreate(true)} icon="add" disabled={!selectedTemplateId}>
                        Add Attribute
                    </Button>
                }
            />
            ...
            <Card title="Attribute Register" subtitle="Attribute definitions for the active template" action={<InfoTip text="Keep option labels concise and validation rules focused on actual data quality checks." />} padding="none">
                <DataTable
                    ...
                    emptyIcon="tune"
                    onRowClick={(row) => navigate(`/attributes/${row.id}?templateId=${selectedTemplateId}`)}
                />
            </Card>
